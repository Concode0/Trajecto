...
class ConfirmationScreen(ModalScreen):
    """Screen to confirm a destructive action."""
                ...
                def check_delete(delete: bool) -> None:
                    if delete:
                        try:
                            ...
                            # Delete the old dataset and create a new one
                            ...
                            self.notify(f"Error deleting row: {e}", severity="error", timeout=5)
                
                self.push_screen(
                    ConfirmationScreen(f"Are you sure you want to delete row {row_index}?"),
                    check_delete
                )
                ...
                if path in self._file and isinstance(self._file[path], h5py.Group):
                    ...
                    def check_delete(delete: bool) -> None:
                        if delete:
                            try:
                                del self._file[path]
                                self.notify(f"Deleted sample '{selected_item}'", timeout=2)
                                # Refresh the list
                                self._column1.update_list(self.add_dir_metadata(), 0)
                            except Exception as e:
                                self.notify(f"Error deleting sample: {e}", severity="error", timeout=5)

                    self.push_screen(
                        ConfirmationScreen(f"Are you sure you want to delete '{selected_item}'?"),
                        check_delete
                    )
